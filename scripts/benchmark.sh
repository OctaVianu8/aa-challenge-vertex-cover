...
echo "Building..."
make -C "$PROJECT_ROOT" bin/brute-force bin/greedy-highest-order bin/greedy-remove-edges bin/test-gen-random > /dev/null 2>&1

# Create benchmark directory
mkdir -p "$PROJECT_ROOT/benchmark-graphs"

# Function to benchmark an algorithm
benchmark_algo() {
    ALGO=$1
    OUTPUT_FILE="$PROJECT_ROOT/benchmark-graphs/${ALGO}.dat"
    ...
    for N in $(seq 5 $MAX_N); do
        ...
        for run in $(seq 1 $RUNS_PER_N); do
            ...
            "$BIN_DIR/test-gen-random" $DENSITY $N > /tmp/bench_test.in 2>/dev/null

            # Time the algorithm (in milliseconds)
            start=$(date +%s%N)
            "$BIN_DIR/$ALGO" < /tmp/bench_test.in > /dev/null 2>&1
            end=$(date +%s%N)
...
echo "Done! Data saved to $PROJECT_ROOT/benchmark-graphs/"
echo "Run 'python3 $PROJECT_ROOT/plot-benchmark.py' to generate graphs"
